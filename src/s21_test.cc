...
TEST(WithController, Controller) {
  ...
  std::string result =
      controller.startCommonCalculator(expression, s21::Model::X);
...
TEST(AnnuitetCreditWithController, Controller) {
  ...
  std::vector<double> result =
      controller.startCreditCalculator(1, 1000, 12, 9.5);
...
TEST(DifferCreditWithController, Controller) {
  Controller controller;
  std::vector<double> result =
      controller.startCreditCalculator(2, 1000, 12, 9.5);
  EXPECT_EQ(result.size(), 3);
}
...
TEST_F(ModelTest, s21_calc_just_double_numb_test) {
  ...
  std::string result = model.commonCalcStart(expression, s21::Model::X);
...
TEST_F(ModelTest, s21_calc_sin_cos_1_test) {
  ...
  for (double number = -2 * M_PI; number <= 2 * M_PI; number += M_PI_2) {
    ...
    EXPECT_EQ(result1,
              s21::Model::zeroIsZero(std::to_string(std::sin(number))));
    EXPECT_EQ(result2,
              s21::Model::zeroIsZero(std::to_string(std::cos(number))));
  }
}
...
TEST_F(ModelTest, s21_calc_sin_cos_3_test) {
  ...
  std::string result1 = model.commonCalcStart(expression1, s21::Model::X);
  std::string result2 = model.commonCalcStart(expression2, s21::Model::X);
  EXPECT_EQ(result1,
            s21::Model::zeroIsZero(std::to_string(std::sin(12345.87654321))));
  EXPECT_EQ(result2,
            s21::Model::zeroIsZero(std::to_string(std::cos(12345.87654321))));
}
...
TEST_F(ModelTest, s21_calc_sin_cos_4_test) {
  ...
  std::string result2 = model.commonCalcStart(expression2, x);
  EXPECT_EQ(result1,
            s21::Model::zeroIsZero(std::to_string(std::sin(M_PI * 100))));
  EXPECT_EQ(result2,
            s21::Model::zeroIsZero(std::to_string(std::cos(M_PI * 100))));
}
//
...
TEST_F(ModelTest, s21_calc_tan_atan_1_test) {
  ...
  for (double number = -2 * M_PI; number <= 2 * M_PI; number += M_PI) {
    ...
    std::string result2 = model.commonCalcStart(expression2, x);
    EXPECT_EQ(result1,
              s21::Model::zeroIsZero(std::to_string(std::tan(number))));
    EXPECT_EQ(result2,
              s21::Model::zeroIsZero(std::to_string(std::atan(number))));
  }
}
...
TEST_F(ModelTest, s21_calc_tan_atan_5_test) {
  ...
  std::string result2 = model.commonCalcStart(expression2, x);
  EXPECT_EQ(result1,
            s21::Model::zeroIsZero(std::to_string(std::tan(-M_PI * 100))));
  EXPECT_EQ(result2,
            s21::Model::zeroIsZero(std::to_string(std::atan(-M_PI * 100))));
}
...
TEST_F(ModelTest, s21_calc_comp_good_6_test) {
  ...
  std::string result = model.commonCalcStart(expression, s21::Model::X);
  double answer = std::sin(1.0) + std::log(10.0) - std::log10(100.0) +
                  std::fmod(std::pow(2.0, 3.0), 5.0);  // 4.143
  EXPECT_EQ(result, s21::Model::zeroIsZero(std::to_string(answer)));
}

TEST_F(ModelTest, s21_calc_comp_good_7_test) {
  std::string expression =
      "cos(0.5) * log(1000.0) - sqrt(25.0) + 2.0^4.0 mod 3.0";
  std::string result = model.commonCalcStart(expression, s21::Model::X);
  double answer = std::cos(0.5) * std::log10(1000.0) - std::sqrt(25.0) +
                  std::fmod(std::pow(2.0, 4.0), 3.0);  // -1.369
  EXPECT_EQ(result, s21::Model::zeroIsZero(std::to_string(answer)));
}

TEST_F(ModelTest, s21_calc_comp_good_8_test) {
  std::string expression =
      "(log(10000.0) - sin(0.75) + sqrt(16.0) + 3.0^3.0) mod 7.0";
  std::string result = model.commonCalcStart(expression, s21::Model::X);
  double answer = std::fmod(
      log10(10000.0) - std::sin(0.75) + std::sqrt(16.0) + (std::pow(3.0, 3.0)),
      7.0);  // 6.319
  EXPECT_EQ(result, s21::Model::zeroIsZero(std::to_string(answer)));
}
