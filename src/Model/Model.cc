...
std::vector<double> Model::creditCalcStart(int type, double sum_d,
...
std::string Model::commonCalcStart(const std::string &expression,
                                   const std::string &x) {
...
std::string Model::convertToPosifix(const std::string &expression,
                                    const std::string &x) {
  bool flag_status = true;
  std::string posifix;
  std::list<std::string> operatorList;

  for (size_t infix_count = 0; infix_count < expression.size() && flag_status;
       ++infix_count) {
    ...
    if (isdigit(inf_letter)) {
      ...
      flag_status =
          handleOperator(posifix, expression, infix_count, operatorList);
    }
  }
...
void Model::handleDigit(std::string &posifix, const std::string &expression,
                        size_t &infix_count) {
...
bool Model::handleDote(std::string &posifix, const std::string &expression,
                       size_t &infix_count) {
  bool status = true;
  if (infix_count > 0 && isdigit(expression.at(infix_count - 1)) &&
      isdigit(expression.at(infix_count + 1))) {
    posifix.push_back(expression.at(infix_count));
  } else {
...
bool Model::handleXP(std::string &posifix, const std::string &x,
                     char inf_letter) {
  bool status = true;
  if (!isStringNumber(x)) {
...
bool Model::handleOperator(std::string &posifix, const std::string &expression,
                           size_t &infix_count,
                           std::list<std::string> &operatorList) {
  bool flag_status = true;
  ...
  if (operat != "!") {
    if (isUnarOperator(expression, infix_count)) {
      ...
      while (!operatorList.empty() &&
             getPriority(operat) < getPriority(operatorList.back())) {
...
std::string Model::calculateResult(const std::string &posifix,
                                   const std::string &x) {
  ...
  for (const auto &token : tokens) {
    if (flag_status) {
      if (token == "p") {
        ...
        flag_status = calculateMagicWithOperator(
            const_cast<std::string &>(token), doubleList);
      }
    }
  }
...
bool Model::calculateMagicWithOperator(std::string &token,
                                       std::list<double> &doubleList) {
  bool flag_status = true;
...
bool Model::isStringNumber(const std::string &expression) {
  bool is_number = true;
  bool has_dot = false;
  if (!(isdigit(expression.front()) || expression.front() == 'x' ||
        (expression.front() == '-' && expression.size() != 1))) {
    is_number = false;
  }
...
bool Model::isUnarOperator(std::string infix, size_t count) {
  ...
  if (infix.at(count) == '+' || infix.at(count) == '-') {
    if (((count > 0 && (isOneLetterOperator(infix.at(count - 1)) ||
                        infix.at(count - 1) == '(')) ||
         count == 0) &&
        (count + 1 < infix.size() &&
         (isdigit(infix.at(count + 1)) || infix.at(count + 1) == 'x' ||
          infix.at(count + 1) == 'p'))) {
      isUnar = true;
    }
  }
...
bool Model::isEngineeringFunction(const std::string &operat) {
  return operat == "sin" || operat == "cos" || operat == "tan" ||
         operat == "acos" || operat == "asin" || operat == "atan" ||
         operat == "ln" || operat == "log" || operat == "sqrt";
}
...
bool Model::doTwoOperator(std::string &operat, double operand1, double operand2,
                          double *answer) {
  bool flag_status = true;
  if (operat == "+") {
...
bool Model::doOneOperator(std::string &operat, double operand, double *answer) {
  bool flag_status = true;
  if (std::fabs(operand) >
      MAX_NUMBER) {  //  область определения от -1000000 до 1000000
    ...
    *answer = std::tan(operand);
    ...
    if (fabs(operand) - 1.0 >
        s21_EPS_TEST) {  // значения х могут быть только в диапазоне [-1;1]
  ...
  if (flag_status && std::fabs(*answer) > MAX_NUMBER) {
    std::cout << "Result of one of function  is too much small/big"
              << std::endl;
    flag_status = false;
  }
  return flag_status;
...
bool Model::isNextLetterDigitableOrEmpty(const std::string &infix,
                                         size_t infix_count) {
  bool status = false;
  if (infix_count == infix.size() - 1) {
    status = true;
  } else if (infix_count + 1 < infix.size() &&
             (!(isdigit(infix.at(infix_count + 1)) ||
                infix.at(infix_count + 1) == '.'))) {
    status = true;
  }
  return status;
}

bool Model::findOpenBracket(std::string &posifix,
                            std::list<std::string> &operatorList,
                            bool isCloseBracket) {
  bool status = true;
...
void Model::popTopOperation(std::string &posifix,
                            std::list<std::string> &operatorList) {
